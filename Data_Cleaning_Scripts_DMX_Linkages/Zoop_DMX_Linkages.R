...
SewardLineMayCopepods <- MayZo %>%
               mutate(SewardLineMayCopepods = rowSums(MayZo[,2:35], na.rm=T)) %>% # sum all small & large copepods
               select(Year, SewardLineMayCopepods)

SewardLineMayEuphausiids <- MayZo %>%
                  rename(SewardLineMayEuphausiids = Euphausiids) %>%
                  select(Year, SewardLineMayEuphausiids)
